...
import { useGetProductsByIdQuery, useUpdateProductMutation } from "@/api/product";
...
const EditProduct = () => {
  const { id } = useParams();
  const [updateProductMutation, { reset, isLoading: isAddLoading }] = useUpdateProductMutation();
  ...
  const onFinish = async (values: IProduct) => {
    ...
    try {
      await updateProductMutation({...productData, id: Number(id)}).unwrap();
